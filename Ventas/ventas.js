...
async function buscarArticuloContado() {
  ...
  let productos = await getItems("user/"+id_user+"/articulo");
  ...
  btnsSeleccionar.forEach((btn) =>
    btn.addEventListener("click", async (e) => {
      try {
        ...
        var_id = id;
        let articulo = await getItems("user/"+id_user+"/articulo");
...
if (nombre != "" && total > 0) {
          ...
          if (cantidad > 0 && cantidad <= cant) {
            ...
            if (total > 0) {
               let EstadoCaja = await getItems("user/"+id_user+"/EstadoDeCaja");
            let totalViejo;
            ...
            let idEstado = "1";
            await modificarItem("user/"+id_user+"/EstadoDeCaja", idEstado, nuevoDato);

            await IngresarDatos("user/"+id_user+"/ventas", {
              tipoVentaId: 1,
              articuloId: artId,
              cantidad: cantidad,
              totalVenta: total,
            ...
            await modificarItem("user/"+id_user+"/articulo", artId, newData);
          ...
          if (nombreCliente != "" ) {
             if (nombre != "") {
                  ...
                  if (cantidad > 0 && cantidad <= cant ) {
                    ...
                    await modificarItem ("user/"+id_user+"/EstadoDeCaja" , idEstado , newEstado);
                    ...
                    let article = await IngresarDatos("user/"+id_user+"/ventas", newVenta );
                    ...
                    let restaDeCantidad = await modificarItem("user/"+id_user+"/articulo" , artId , ArtiModificado );

                    let cuenta = await getItems("user/"+id_user+"/cuentaCorriente");
                    ...
                    await modificarItem("user/"+id_user+"/cuentaCorriente" , idCuenta ,
                      ...
                      await IngresarDatos("user/"+id_user+"/cuentaCorriente", {
                        cliente: nombreCliente,
                        clientesId: clienteid,
                        deuda: total,
                      });
...
async function aceptarVentaTarjeta() {
  ...
  if (nombre != "") {
    if (cantidad > 0 && cantidad <= cant  ) {
      if (total > 0) {
         try {
      let totales = await getItems("user/"+id_user+"/EstadoDeCaja");
      ...
      let total1 = parseInt(totalViejo) + parseInt(total);
       ...
       await modificarItem("user/"+id_user+"/EstadoDeCaja", "3", {
      ...
      await modificarItem("user/"+id_user+"/articulo" , artId ,
        {
          nombre: nombre,
          cantidad: nuevaCantidad,
          PrecioCompra: PC,
